...
async fn init() -> &'static TestConfig {
    PROXY
        .get_or_init(|| async {
            ...
            let key = {
                let mut m = state.api_key_manager.lock().unwrap();
                m.generate_key("e2e-test")
            };
            ...
            TestConfig {
                proxy_url: format!("http://127.0.0.1:{}", p),
                api_key: key,
...
async fn traces(cfg: &TestConfig) -> reqwest::Response {
    cli()
        .get(format!("{}/v1/traces", cfg.proxy_url))
        .header("Authorization", format!("Bearer {}", cfg.api_key))
        .send()
        .await
        .unwrap()
...
async fn e2e_full_suite() {
    ...
    macro_rules! S {
        ($n:expr,$t:expr,$b:expr) => {{
            ...
            match tokio::time::timeout(Duration::from_secs($t), $b).await {
                Ok(v) => {
                    ok += 1;
                    println!("✅ ({:.0}s) {:?}", start.elapsed().as_secs_f32(), v);
                }
                Err(_) => {
                    fail += 1;
                    println!("⏰ 超时(>{}s)", $t);
    ...
    S!("S01 健康检查", 5, async {
        cli()
            .get(format!("{}/health", cfg.proxy_url))
            .send()
            .await
            .map(|r| r.status())
    });
    ...
    S!("S02 模型列表", 5, async {
        cli()
            .get(format!("{}/models", cfg.proxy_url))
            .header("Authorization", format!("Bearer {}", cfg.api_key))
            .send()
            .await
            .map(|r| r.status())
    ...
    S!("S06 Passthrough直通", 120, async {
        chat(cfg, vec![]).await.status()
    });
    ...
    S!("S07 治理+签名+存库", 120, async {
        ...
        let tid = r
            .headers()
            .get("VERIDACTUS-Trace-Id")
            .and_then(|v| v.to_str().ok().map(|s| s.to_string()));
        let pl = r
            .headers()
            .get("VERIDACTUS-Proof-Levels")
            .and_then(|v| v.to_str().ok().map(|s| s.to_string()));
    ...
    S!("S09 审计令牌错误详情", 15, async {
        let r = cli()
            .post(format!("{}/v1/chat/completions", cfg.proxy_url))
            .header("Authorization", "Bearer bad")
            .header("VERIDACTUS-Audit-Token", cfg.audit_token)
            .header("Content-Type", "application/json")
            .json(&serde_json::json!({"model":MODEL,"messages":[{"role":"user","content":"hi"}]}))
            .send()
            .await
        ...
        (
            ...
            r.json::<serde_json::Value>()
                .await
                .ok()
                .and_then(|j| j["error"]["details"].as_object().map(|_| true)),
        )
    });
