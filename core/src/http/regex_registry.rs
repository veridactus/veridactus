...
pub fn id_card_re() -> &'static Regex {
    ID_CARD_RE.get_or_init(|| Regex::new(r"[1-9]\d{5}(18|19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}[\dXx]").unwrap())
}
...
pub fn api_key_re() -> &'static Regex {
    API_KEY_RE.get_or_init(|| Regex::new(r"(?i)(sk-[a-zA-Z0-9]{20,}|api[_-]?key[=:]\s*[a-zA-Z0-9]{16,})").unwrap())
}
...
pub fn ignore_instructions_re() -> &'static Regex {
    IGNORE_INST_RE.get_or_init(|| Regex::new(r"(?i)ignore\s+(all\s+)?(previous|above)\s+instructions").unwrap())
}

pub fn you_are_now_re() -> &'static Regex {
    YOU_ARE_NOW_RE.get_or_init(|| Regex::new(r"(?i)you\s+are\s+now\s+(DAN|unrestricted|a\s+different)").unwrap())
}

pub fn system_prompt_re() -> &'static Regex {
    SYSTEM_PROMPT_RE.get_or_init(|| Regex::new(r"(?i)(system\s+prompt|developer\s+mode|bypass\s+safety)").unwrap())
}

pub fn reveal_prompt_re() -> &'static Regex {
    REVEAL_PROMPT_RE.get_or_init(|| Regex::new(r"(?i)reveal\s+(your|the)\s+(system\s+)?(prompt|instructions)").unwrap())
}

pub fn jailbreak_re() -> &'static Regex {
    JAILBREAK_RE.get_or_init(|| Regex::new(r"(?i)(jailbreak|do\s+anything\s+now|ignore\s+all)").unwrap())
}

pub fn goal_hijack_re() -> &'static Regex {
    GOAL_HIJACK_RE.get_or_init(|| Regex::new(r"(?i)(your\s+new\s+goal\s+is|primary\s+objective\s+is\s+now)").unwrap())
}
...
pub fn shell_injection_re() -> &'static Regex {
    SHELL_INJ_RE.get_or_init(|| Regex::new(r"(?i)(rm\s+-rf|sudo\s+|chmod\s+777|wget\s+http|curl\s+.*\|\s*bash)").unwrap())
}

pub fn sql_injection_re() -> &'static Regex {
    SQL_INJ_RE.get_or_init(|| Regex::new(r"(?i)(UNION\s+SELECT|DROP\s+TABLE|1\s*=\s*1|' OR '1'='1)").unwrap())
}

pub fn destructive_cmd_re() -> &'static Regex {
    DESTRUCTIVE_CMD_RE.get_or_init(|| Regex::new(r"(?i)(docker\s+rm\s+-f|kubectl\s+delete|terraform\s+destroy)").unwrap())
}
