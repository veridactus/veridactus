...
async function api<T=any>(url:string,opts:RequestInit={}):Promise<T|null>{try{const r=await fetch(url,opts);if(!r.ok)return null;return r.json();}catch{return null;}}
...
export default function ChatPage() {
  ...
  useEffect(()=>{if(token)api<{conversations:Conversation[]}>('/api/v1/conversations',{headers:{Authorization:`Bearer ${token}`}}).then(d=>d&&setConvs(d.conversations));},[token]);
  ...
  // New Chat
  const newChat = useCallback(async()=>{if(streaming||creatingRef.current)return;creatingRef.current=true;
    try{setMessages([]);setActiveId(null);save(CACHE_CONV,null);save(CACHE_MSGS,[]);
      if(token){const c=await api<Conversation>('/api/v1/conversations',{method:'POST',headers:jsonHeaders,body:JSON.stringify({title:'新对话',model:selModel.id})});
        if(c){setActiveId(c.id);save(CACHE_CONV,c.id);api<{conversations:Conversation[]}>('/api/v1/conversations',{headers}).then(d=>d&&setConvs(d.conversations));}}}
    finally{creatingRef.current=false;}
  },[streaming,token,selModel.id]);

  // Switch
  const switchConv = useCallback(async(c:Conversation)=>{if(streaming)return;
    setActiveId(c.id);save(CACHE_CONV,c.id);
    const d=await api<{conversation:Conversation;messages:any[]}>('/api/v1/conversations/'+c.id,{headers});
    if(d){const ms=d.messages.map((m:any)=>({id:m.id,role:m.role,content:m.content,model:m.model||'',tokens:m.tokens||0,timestamp:Date.now()}));setMessages(ms);save(CACHE_MSGS,ms);
      if(d.conversation?.model){const fm=modelsRef.current.find(x=>x.id===d.conversation.model);if(fm)setSelModel(fm);}}},[streaming,token]);

  // Delete
  const delConv = useCallback(async(id:string,e:React.MouseEvent)=>{e.stopPropagation();if(!token)return;
    await fetch('/api/v1/conversations/'+id,{method:'DELETE',headers});
    setConvs(p=>p.filter(x=>x.id!==id));if(activeId===id){setMessages([]);setActiveId(null);save(CACHE_CONV,null);save(CACHE_MSGS,[]);}},[token,activeId]);

  // Send
  const send = useCallback(async()=>{
    if(!input.trim()||streaming)return;
    const uc=input.trim();setInput('');
    ...
    if(!cid&&token&&!creatingRef.current){const title=uc.length>30?uc.slice(0,30)+'...':uc;
      const c=await api<Conversation>('/api/v1/conversations',{method:'POST',headers:jsonHeaders,body:JSON.stringify({title,model:selModel.id})});
      if(c){cid=c.id;setActiveId(cid);save(CACHE_CONV,cid);api<{conversations:Conversation[]}>('/api/v1/conversations',{headers}).then(d=>d&&setConvs(d.conversations));}}
    if(cid&&token)fetch('/api/v1/conversations/'+cid+'/messages',{method:'POST',headers:jsonHeaders,body:JSON.stringify({id:u.id,role:'user',content:uc,model:selModel.id,tokens:0,timestamp:u.timestamp})}).catch(()=>{});

    const ctrl=new AbortController();abortRef.current=ctrl;
    try{
      const res=await fetch('/v1/chat/completions',{method:'POST',signal:ctrl.signal,headers:jsonHeaders,
      ...
      if(cid&&token){fetch('/api/v1/conversations/'+cid+'/messages',{method:'POST',headers:jsonHeaders,body:JSON.stringify({id:a.id,role:'assistant',content:fc,model:selModel.id,tokens:tk,timestamp:Date.now()})}).catch(()=>{});
        const title=uc.length>30?uc.slice(0,30)+'...':uc;fetch('/api/v1/conversations/'+cid,{method:'PUT',headers:jsonHeaders,body:JSON.stringify({title})}).catch(()=>{});
            ...
            {messages.map(msg=>(
              <div key={msg.id} className={`flex gap-3 mb-6 ${msg.role==='user'?'flex-row-reverse':''}`}>
                ...
                <div className={`min-w-0 ${msg.role==='user'?'max-w-[80%]':'max-w-[85%]'}`}>
                  <div className={`px-4 py-3 rounded-2xl text-[14px] leading-[1.65] ${
                    msg.role==='user'
                      ?'bg-[#6c5ce7]/15 border border-[#6c5ce7]/20 text-[#e2e8f0] rounded-tr-lg'
                      :'bg-white/[0.025] border border-white/[0.05] text-[#d4dce6] rounded-tl-lg'}`}>
                    {/* 🔧 核心修复：长内容自动换行 + 代码等宽 + 诗歌保留换行 */}
                    <div className="whitespace-pre-wrap break-words [overflow-wrap:anywhere] max-w-full">
                      {msg.content||(msg.role==='assistant'&&streaming?<span className="inline-flex gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#6c5ce7] animate-pulse"/><span className="w-1.5 h-1.5 rounded-full bg-[#6c5ce7] animate-pulse" style={{animationDelay:'0.15s'}}/><span className="w-1.5 h-1.5 rounded-full bg-[#6c5ce7] animate-pulse" style={{animationDelay:'0.3s'}}/></span>:null)}
                    </div>
                  </div>
                  {/* Metadata footer */}
        ...
        <div className="border-t border-white/[0.04] bg-[#0b0f19]/95 backdrop-blur-md px-4 py-3 flex-shrink-0">
          <div className="max-w-[768px] mx-auto flex items-end gap-2.5">
            ...
            {streaming?(
              ...
              <button onClick={send} disabled={!input.trim()} className="w-[42px] h-[42px] rounded-xl bg-[#5b4ae0] hover:bg-[#6c5ce7] disabled:bg-white/[0.04] disabled:cursor-not-allowed flex items-center justify-center flex-shrink-0 transition-all duration-150 cursor-pointer border-none"><Send size={15} className="text-white disabled:text-[#4a5568]"/></button>
            )}
          </div>
          <div className="max-w-[768px] mx-auto mt-2">
            <p className="text-[10px] text-[#3a4568] text-center">VERIDACTUS 安全沙箱 · L0 密码学签名 · 数据不可篡改</p>
          </div>
        </div>
      </div>
    </div>
