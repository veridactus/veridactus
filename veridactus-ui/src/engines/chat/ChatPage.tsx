...
import { Send, Shield, Zap, ChevronDown, Columns, Plus, Trash2, MessageSquare, PanelLeftClose, PanelLeft } from 'lucide-react';
...
const MSG_CACHE = 'veridactus_chat_messages';
...
async function createConversation(token:string,title:string,model:string): Promise<Conversation|null> {
...
async function deleteConversationAPI(token:string,id:string): Promise<boolean> {
  try { await fetch('/api/v1/conversations/'+id,{method:'DELETE',headers:{Authorization:`Bearer ${token}`}}); return true; } catch { return false; }
}
async function updateConversationTitle(token:string,id:string,title:string) {
  try { await fetch('/api/v1/conversations/'+id,{method:'PUT',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({title})}); } catch {}
}
async function saveMessageToServer(token:string,convId:string,msg:Message) {
  try { await fetch('/api/v1/conversations/'+convId+'/messages',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({id:msg.id,role:msg.role,content:msg.content,model:msg.model||'',tokens:msg.tokens||0,timestamp:msg.timestamp})}); } catch {}
}
...
export default function ChatPage() {
  ...
  const handleNewChat = useCallback(async () => {
    ...
    if (token) {
      const conv = await createConversation(token, '新对话', selectedModel.id);
      if (conv) { setActiveConvId(conv.id); saveConvId(conv.id); const updated = await fetchConversations(token); setConversations(updated); }
    }
  ...
  const handleSwitchConv = useCallback(async (conv: Conversation) => {
    if (isStreaming) return;
    setActiveConvId(conv.id); saveConvId(conv.id); saveMsgs([]);
    setMessages([]); setModels([]); // Reset to show loading state
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.conversation?.model && models.length>0) {
          const m = models.find(x=>x.id===d.conversation.model);
  ...
  const handleSend = useCallback(async () => {
    ...
    const assistantMsg: Message = { id:genId(), role:'assistant', content:'', model:selectedModel.id, timestamp:Date.now() };
    const newMsgs = [...messages, userMsg, assistantMsg];
    setMessages(newMsgs); setInput(''); setIsStreaming(true);

    // Auto-create conversation if needed + sync to server
    let convId = activeConvId;
    if (!convId && token) {
      const title = userMsg.content.length>30?userMsg.content.slice(0,30)+'...':userMsg.content;
      const conv = await createConversation(token, title, selectedModel.id);
      if (conv) { convId = conv.id; setActiveConvId(conv.id); saveConvId(conv.id);
        const updated = await fetchConversations(token); setConversations(updated); }
    }
    if (convId && token) { saveMessageToServer(token, convId, userMsg); }
    ...
    try {
      ...
      setMessages(prev=>prev.map(m=>m.id===assistantMsg.id?{...m,content:fullContent,tokens:totalTokens,safety:'safe'}:m));
      if (convId && token) {
        saveMessageToServer(token, convId, {...assistantMsg, content:fullContent, tokens:totalTokens});
        // Auto-update conversation title from first exchange
        const title = userMsg.content.length>30?userMsg.content.slice(0,30)+'...':userMsg.content;
        updateConversationTitle(token, convId, title);
          ...
          <div className="max-w-[800px] mx-auto flex gap-2 items-center">
            ...
            <div className="flex-1 relative">
              <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKeyDown}
                ...
                className="w-full p-2.5 pr-10 rounded-btn bg-white/[0.04] border border-white/[0.08] text-sm text-[#e2e8f0] outline-none resize-none font-sans"
